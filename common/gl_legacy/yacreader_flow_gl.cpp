...
void YACReaderFlowGL::insert(const char *name, GLuint texture, float x, float y,int item)
{
    startAnimationTimer();
...
void YACReaderFlowGL::replace(const char *name, GLuint texture, float x, float y,int item)
{
    startAnimationTimer();
...
void YACReaderFlowGL::reset()
{
    ...
    for(int i = 0;i<numObjects;i++){
        if(images[i].texture != defaultTexture)
            deleteTexture(images[i].texture);
    }
...
void YACReaderComicFlowGL::updateImageData()
{
	...
	if( idx >= 0 && !worker->result().isNull())
	{
		if(!loaded[idx])
		{
            ...
            QString s = "cover";
            replace(s.toLocal8Bit().data(), cover, x, y,idx);
